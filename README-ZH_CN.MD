# 抽奖程序
##### 【文件模式】
* 1、【文件模式】年会抽奖程序，3D 球体抽奖，支持奖品信息配置，参与抽奖人员信息`Excel`导入，抽奖结果`Excel`导出 
##### 【数据库模式】
* 2、【数据库模式】或通过Mysql动态用户数据进行实时锁定实时加载用户数据

> 通过动态检测，如果没有检测到数据库信息就自动走Excel
检测到Mysql链接就走Mysql
...
##### 技术：Node + Express + Mysql2 + PyzSecurity + Three.js
